...
rc_deptree_update_needed(time_t *newest, char *file)
{
	...
	newer |= !deep_mtime_check(rc_dirfd(RC_DIR_SYSCONF), "openrc.conf", true, &mtime, file);
	newer |= !deep_mtime_check(rc_dirfd(RC_DIR_SYSCONF), "openrc.conf.d", true, &mtime, file);
	if (rc_is_user()) {
		newer |= !deep_mtime_check(rc_dirfd(RC_DIR_USRCONF), "openrc.conf", true, &mtime, file);
		newer |= !deep_mtime_check(rc_dirfd(RC_DIR_USRCONF), "openrc.conf.d", true, &mtime, file);
	}
