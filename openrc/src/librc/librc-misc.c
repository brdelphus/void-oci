...
{
	RC_STRINGLIST *conf = config_load(rc_dirfd(dir), "openrc.conf");
...
rc_conf_value(const char *setting)
{
	...
	if (rc_is_user()) {
		...
		rc_config_directory(rc_conf, rc_dirfd(RC_DIR_USRCONF), "openrc.conf.d");
	}
	...
	rc_config_directory(rc_conf, rc_dirfd(RC_DIR_SYSCONF), "openrc.conf.d");
